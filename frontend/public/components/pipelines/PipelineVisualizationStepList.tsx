  ...
  if (status === runStatus.Succeeded || status === runStatus.Failed) {
    ...
    return (
      <div style={{ color: getRunStatusColor(status).pftoken.value }}>
        <svg {...sharedProps}>
          <circle className="odc-pipeline-vis-steps-list__icon-backdrop" cx={size / 2} cy={size / 2} r={size / 2 - 1} />
          {icon}
        </svg>
      </div>
    );
...
// export const PipelineVisualizationStepList: React.FC<PipelineVisualizationStepListProps> = ({ isSpecOverview, taskName, steps }) => (
export const PipelineVisualizationStepList: React.ComponentType<PipelineVisualizationStepListProps> = ({ isSpecOverview, taskName, steps }) => (
    ...
    {steps.map(({ duration, name, runStatus: status }) => {
      return (
        <div className="odc-pipeline-vis-steps-list__step" key={name}>
          ...
          {!isSpecOverview && <div className="odc-pipeline-vis-steps-list__duration">{duration}</div>}
        </div>
      );
    })}
  </div>
