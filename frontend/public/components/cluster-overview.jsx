...
// const DashboardLink = ({ to, id }) => (
//   <Link id={id} className="co-external-link" target="_blank" to={to}>
//     View Grafana Dashboard
//   </Link>
// );

const Graphs = requirePrometheus(
  connectToURLs(MonitoringRoutes.AlertManager)(({ namespace, isOpenShift, urls }) => {
        ...
        <div className="group">
          ...
          <div className="container-fluid group__body">
            ...
            <div className="row">
              ...
              <div className="col-md-3 col-sm-6">
                <Status title={t('CONTENT:CRASHLOOPING') + ' ' + t('RESOURCE:POD')} name={t('RESOURCE:POD')} query={`count(increase(kube_pod_container_status_restarts_total${namespace ? `{namespace="${namespace}"}` : ''}[5m]) > 5 )`} href={`/k8s/${namespace ? `ns/${namespace}` : 'all-namespaces'}/pods?rowFilter-pod-status=CrashLoopBackOff`} />
              </div>
            </div>
          </div>
        </div>
...
const GraphsPage = ({ fake, limited, namespace, openshiftFlag }) => {
  ...
  return (
    <Firehose resources={resources}>
      <StatusBox label="Namespaces">{body}</StatusBox>
    </Firehose>
  );
};
...
const permissionedLoader = () => {
  ...
  return coFetchJSON(`${prometheusBasePath}/api/v1/query?query=${encodeURIComponent(q)}`).then(
    () => AllGraphs,
    err => {
