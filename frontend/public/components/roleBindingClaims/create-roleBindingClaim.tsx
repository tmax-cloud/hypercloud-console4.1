...
class RoleBindingClaimFormComponent extends React.Component<RoleBindingClaimProps_, RoleBindingClaimState_> {
  ...
  getNamespaceList() {
    ...
    k8sList(ko)
      ...
      .then(
        data => {
          let namespaceList = data.map(cur => {
            return {
              name: cur.metadata.name,
            };
          });
  ...
  save(e) {
    ...
    (this.props.isCreate ? k8sCreate(ko, roleBindingClaim) : k8sUpdate(ko, roleBindingClaim, metadata.namespace, roleBindingClaim.metadata.name)).then(
      () => {
  ...
  render() {
    ...
    let namespaceOptions = namespaceList.map(cur => {
      return (
        <option value={cur.name} data-content="<ResourceIcon kind='Container'/>">
          {cur.name}
        </option>
      );
    });
      ...
      <div className="co-m-pane__body">
        ...
        <form className="co-m-pane__body-group co-create-secret-form" onSubmit={this.save}>
          ...
          <fieldset disabled={!this.props.isCreate}>
            {/* Name */}
            <FirstSection label={t('CONTENT:NAME')} children={<input className="form-control" type="text" onChange={this.onNameChanged} value={this.state.roleBindingClaim.metadata.name} required />} id="name" />

            {/* Resource Name */}
            <FirstSection label={t('CONTENT:RESOURCENAME')} children={<input className="form-control" type="text" onChange={this.onResourceNameChanged} value={this.state.roleBindingClaim['resourceName']} required />} id="resourceName" />
            ...
            <ButtonBar errorMessage={this.state.error} inProgress={this.state.inProgress}>
              ...
              <Link to={formatNamespacedRouteForResource('rolebindingclaims')} className="btn btn-default" id="cancel">
                {t('CONTENT:CANCEL')}
              </Link>
            </ButtonBar>
          </fieldset>
...
export const CreateRoleBindingClaim = ({ match: { params } }) => {
  ...
  return <RoleBindingClaimFormComponent t={t} fixed={{ metadata: { namespace: params.ns } }} roleBindingClaimTypeAbstraction={params.type} explanation={pageExplanation[params.type]} titleVerb="Create" isCreate={true} />;
};
