...
const packagesFor: PackagesFor = configMap => (_.get(configMap, 'data.packages') ? safeLoad(_.get(configMap, 'data.packages', '')) : []);

type ClusterServiceVersionsFor = (configMap: ConfigMapKind) => ClusterServiceVersionKind[];
const clusterServiceVersionsFor: ClusterServiceVersionsFor = configMap => (_.get(configMap, 'data.clusterServiceVersions') ? safeLoad(_.get(configMap, 'data.clusterServiceVersions', '')) : []);

export const PackageHeader: React.SFC<PackageHeaderProps> = props => (
  <ListHeader>
    <ColHead {...props} className="col-sm-4 col-xs-6">
      Name
    </ColHead>
    <ColHead {...props} className="col-sm-4 hidden-xs">
      Latest Version
    </ColHead>
    <ColHead {...props} className="col-sm-4 col-xs-6">
      Subscriptions
...
export const PackageRow: React.SFC<PackageRowProps> = props => {
  ...
  const subscriptionLink = () => (ns !== ALL_NAMESPACES_KEY ? <Link to={`/k8s/ns/${ns}/${SubscriptionModel.plural}/${subscription.metadata.name}`}>View subscription</Link> : <Link to={`/k8s/all-namespaces/${SubscriptionModel.plural}?name=${obj.packageName}`}>View subscriptions</Link>);
  ...
  return (
    <div className="row co-resource-list__item co-package-row">
      ...
      <div className="col-sm-4 col-xs-6 co-package-row__actions">
        {subscription ? subscriptionLink() : <span className="text-muted">Not subscribed</span>}
        {(!subscription || ns === ALL_NAMESPACES_KEY) && (
...
export const PackageList: React.SFC<PackageListProps> = props => (
  <List
    loaded={true}
    ...
    Row={(rowProps: { obj: Package }) => <PackageRow obj={rowProps.obj} currentCSV={props.clusterServiceVersions.find(({ metadata }) => metadata.name === rowProps.obj.channels[0].currentCSV)} catalogSource={props.catalogSource} subscription={props.subscriptions.find(sub => sub.spec.name === rowProps.obj.packageName)} />}
    label="Packages"
    EmptyMsg={() => <MsgBox title="No Packages Found" detail="The catalog author has not added any packages." />}
  />
);

export const CatalogSourceDetails = withFallback<CatalogSourceDetailsProps>(
  ({ obj, configMap, subscription }) => {
...
export const CatalogSourceHeader: React.SFC<CatalogSourceHeaderProps> = props => (
  <ListHeader>
    <ColHead {...props} className="col-md-3" sortField="metadata.name">
      Name
    </ColHead>
    <ColHead {...props} className="col-md-2" sortField="metadata.namespace">
      Namespace
    </ColHead>
    <ColHead {...props} className="col-md-2" sortField="spec.publisher">
      Publisher
    </ColHead>
    <ColHead {...props} className="col-md-2">
      Created
    </ColHead>
  </ListHeader>
);

export const CatalogSourceRow: React.SFC<CatalogSourceRowProps> = props => (
  <ResourceRow obj={props.obj}>
    ...
    <div className="col-md-2">
      <Timestamp timestamp={props.obj.metadata.creationTimestamp} />
    </div>
  </ResourceRow>
);

export const CatalogSourceList = withFallback(
  (props: CatalogSourceListProps) => {
    ...
    return props.loaded ? (
      <React.Fragment>
        ...
        {data.map(obj => (
          <div key={obj.metadata.uid} className="co-catalogsource-list__section">
...
export const CreateSubscriptionYAML: React.SFC<CreateSubscriptionYAMLProps> = props => {
  ...
  const Create = withFallback<CreateProps>(
    createProps => {
      if (createProps.ConfigMap.loaded && createProps.ConfigMap.data) {
        const pkg: Package = (_.get(createProps.ConfigMap.data, 'data.packages') ? safeLoad(_.get(createProps.ConfigMap.data, 'data.packages')) : []).find(({ packageName }) => packageName === new URLSearchParams(props.location.search).get('pkg'));
