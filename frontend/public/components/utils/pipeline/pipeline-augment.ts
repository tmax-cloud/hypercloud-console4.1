...
export const getRunStatusColor = (status: string): StatusMessage => {
  ...
  switch (status) {
    ...
    default:
      return { message: 'PipelineRun not started yet', pftoken: { name: 'sdf', value: '#000000', var: '' } };
  }
};
...
export const getTaskStatus = (pipelinerun: PipelineRun, pipeline: Pipeline): TaskStatus => {
  ...
  const plrTasks = pipelinerun && pipelinerun.status && pipelinerun.status.taskRuns ? Object.keys(pipelinerun.status.taskRuns) : [];
  ...
  if (pipelinerun && pipelinerun.status && pipelinerun.status.taskRuns) {
    ...
    taskStatus[runStatus.Failed] > 0 ? (taskStatus[runStatus.Cancelled] = totalTasks >= plrTaskLength ? totalTasks - plrTaskLength : totalTasks) : (taskStatus[runStatus.Pending] += totalTasks >= plrTaskLength ? totalTasks - plrTaskLength : totalTasks);
