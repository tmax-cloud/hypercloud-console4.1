...
import { formatDuration } from '../../../../public/components/utils/datetime';
import {
  ContainerStatus,
  K8sResourceKind,
  k8sUpdate,
...
import { errorModal } from '../../../../public/components/modals/error-modal';
import {
...
const appendPipelineRunStatus = (pipeline, pipelineRun) => {
  return _.map(pipeline.spec.tasks, task => {
    ...
    const mTask = _.merge(task, {
      status: _.get(
        _.find(pipelineRun.status.taskRuns, { pipelineTaskName: task.name }),
        'status'
      )
    });
...
export const getPipelineTasks = (
  ...
  tasks.forEach(task => {
    if (
      ...
      delete task.resources.inputs[0].from;
    }
  });
...
export const associateServiceAccountToSecret = (
  ...
  k8sGet(ServiceAccountModel, PIPELINE_SERVICE_ACCOUNT, namespace)
    .then(serviceAccount => {
      if (
        _.find(serviceAccount.secrets, s => s.name === secret.metadata.name) ===
        undefined
      ) {
...
export const getSecretAnnotations = (annotation: KeyValuePair) => {
  ...
  if (annotation?.key === SecretAnnotationId.Git) {
    annotations[`${annotationPrefix}/${SecretAnnotationId.Git}-0`] =
      annotation?.value;
  } else if (annotation?.key === SecretAnnotationId.Image) {
    annotations[`${annotationPrefix}/${SecretAnnotationId.Image}-0`] =
      annotation?.value;
  }
  return annotations;
};
