...
volumes: [
  ...
  node(POD_LABEL) {
    ...
    stage('Docker Build'){
      container('docker'){
        withCredentials([usernamePassword(
          ...
          sh "docker build -t ${DOCKER_REGISTRY}/${PRODUCT}:${MAJOR_VERSION}.${MINOR_VERSION}.${PATCH_VERSION}.${HOTFIX_VERSION} -f ./Dockerfile.original ."
    ...
    //     fi
    //     """
    //     sh "kubectl apply -f ./temp-yaml/2.svc-lb.yaml"A
